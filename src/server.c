...
Server *serverConstructor(uint16_t port)
{
...
static inline int makeSocket(unsigned int port)
{
	...
	if (bind(sock, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
		fprintf(stderr, "error: failed to bing socket to 0.0.0.0:%d\n", port);
		exit(1);
	}
...
static inline void handle(Server *server, int fd, fd_set *activeFDs, struct sockaddr_in *addr)
{
	...
	if ((nread = recv(fd, buff, sizeof(buff), 0)) < 0) {
		...
		if (!req) {
			...
			ListNode *handler = server->handlers;
			Response *response = NULL;

			while(handler && !response) {
				response = (*(HandlerP)handler->data)(req);
	...
	close(fd);
	/* delete file discriptor from set */
	FD_CLR(fd, activeFDs);
}
