...
Template *templateConstructor(char *filename)
{
...
{
	template->context = listConstructor(kvConstructor(key, value), sizeof(KV), template->context);
}
...
char *templateRender(Template *template)
{
	...
	size_t r;
	r = fread(buff + 1, sizeof(char), len, file);
	if (!r) {
		fprintf(stderr, "error: read template '%s' error\n", template->filename);
		exit(1);
	}
	...
	for (;;) {
		...
		if (*segment == '{') {
			...
			if (!strncmp(segment, "include", 7)) {
				...
				inc = templateConstructor(segment);
				...
				*spc = '\0';
				...
				fprintf(stderr, "error: unknow exp {%%%s} int '%s' \n", segment, template->filename);		
				exit(1);
			}
		} else {
